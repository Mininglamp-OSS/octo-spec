...
#
# Turns a single task's learning into the right artifact BY KIND, while never
# writing main's rules/ directly. This is the executable backing for the parity
# design §3 (learning reflow) / §6 (task context): the comprehension-gate stays
# the human gate, so a rule only ever becomes a *draft* here, never a committed
# change to rules/ on main.
#
...
#          — pending/ here is used for its ONE legitimate purpose: an undecided,
#            human-review-pending draft, not a dead-letter of finished learnings;
#       2. prints a promotion-issue BODY to stdout (Linked task / proposed rule /
#          COMPREHENSION three questions) for the caller to open a ReviewBot
#          review issue from.
#     It NEVER edits rules/ or rules/_index.yaml. Real promotion = the issue + a
#     reviewed PR + a human merge.
#
...
#   --no-promote             rule: write the draft but suppress the issue body
#                            on stdout (escape hatch to avoid promotion spam).
#
...
LEARNING="$(printf '%s' "$LEARNING" | sed -e '/./,$!d' | sed -e :a -e '/^\n*$/{$d;N;ba}')"
...
if [ "$KIND" = "rule" ]; then
  ...
  if [ -e "$DRAFT_PATH" ] && [ "$SKIP_EXISTING" -eq 1 ]; then
    ...
    # YAML-escape the title/description (quote + escape embedded quotes).
    y_title="${TITLE//\"/\\\"}"
    y_desc="${DESCRIPTION//\"/\\\"}"
...
# DRAFT — generated by $PROG for slug "$SLUG". NOT a committed rule.
# Promotion into rules/ requires a reviewed issue + human merge (comprehension-gate).
id: $RULE_ID
tier: $TIER
priority: $PRIORITY
...
"
  ...
  # Promotion issue body -> stdout (unless suppressed).
  if [ "$NO_PROMOTE" -eq 1 ]; then
    echo "$PROG: --no-promote set; skipping promotion issue body" >&2
    exit 0
  fi
...
**Draft (review, do not auto-merge):** \`$REL_DRAFT\`

> Generated by \`$PROG\`. The draft is NOT a committed rule. Promotion requires
> review here + a reviewed PR + a human merge (comprehension-gate). Assign
> ReviewBot to adjudicate.
...
- [ ] On accept: move draft into \`.octospec/rules/$RULE_ID.md\`, add the
