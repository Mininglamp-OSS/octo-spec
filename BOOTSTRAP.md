...
> 版本已钉死在 `v2.0.0`（可复现、可审计）。不接受请勿继续。
> Following this file makes your agent clone the octo-spec repo to a temp dir and run its
> sync script against THE CURRENT repo. Version is pinned to `v2.0.0`. Stop here if that's not intended.
...
1. 把 octo-spec 浅克隆到临时目录（版本钉死，**勿改 tag**）:
   ...
   git clone --depth 1 --branch v2.0.0 https://github.com/Mininglamp-OSS/octo-spec "$OCTOSPEC_SRC"
   ```
2. 打开 `"$OCTOSPEC_SRC/templates/octospec-init/.claude/skills/octospec-init/SKILL.md"`，
   **照其 5 步在当前仓库执行**（copy → pin → sync → verify → lint），其中：
   ...
   - 第 2 步 manifest 钉 `inherits: octo-spec@2.0.0`
